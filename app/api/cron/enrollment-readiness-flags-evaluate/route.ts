...
export async function GET(req: NextRequest) {
  ...
  // Find all tenants. Feature-flag filtering would ideally check each
  // tenant's license tier — for Stage 4, the helper silently skips any
  // tenant that would fail the tier check at evaluation time (no-op is
  // safe because flags only write rows when conditions hold).
