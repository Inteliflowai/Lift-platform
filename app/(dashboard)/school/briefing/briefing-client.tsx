  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast(t("briefing.toast_load_failed"), "error");
      }
    } finally {
      setLoading(false);
    }
  }, [cycleId, statusFilter, toast, t]);
  ...
  if (!featureEnabled) {
    return (
      <div className="rounded-lg border border-lift-border bg-surface p-6">
        <h1 className="text-2xl font-bold">{t("briefing.page_title")}</h1>
        <p className="mt-2 text-sm text-muted">
          {t("briefing.page_subtitle")}
        </p>
      </div>
    );
  }
    ...
    <div className="space-y-5">
      ...
      {activeSession && (
        <div className="flex flex-col gap-2 rounded-lg border border-primary/40 bg-primary/10 px-4 py-3 text-sm text-primary sm:flex-row sm:items-center sm:justify-between">
          <div className="min-w-0">
            <strong>{t("briefing.active_session_prefix")}</strong> {activeSession.name} · {activeSession.staged} {t("session_page.summary_staged").toLowerCase()}
          </div>
          <Link
          ...
          >
            {t("briefing.active_session_resume")} →
          </Link>
        </div>
      )}
      ...
      <div className="flex flex-col gap-3 rounded-lg border border-lift-border bg-surface p-4 sm:flex-row">
        <label className="flex flex-col gap-1 text-xs text-muted">
          {t("briefing.filter_cycle")}
          <select
            value={cycleId}
            onChange={(e) => setCycleId(e.target.value)}
            className="min-h-[40px] rounded-md border border-lift-border bg-page-bg px-3 py-2 text-sm text-lift-text"
          >
            <option value="all">{t("briefing.filter_all_cycles")}</option>
            {cycles.map((c) => (
              <option key={c.id} value={c.id}>
                {c.name} {c.status === "active" ? `(${t("analytics.all_cycles").toLowerCase().includes("ativ") ? "ativo" : "active"})` : ""}
              </option>
            ))}
          </select>
        </label>
        <label className="flex flex-col gap-1 text-xs text-muted">
          {t("briefing.filter_status")}
          <select
            value={statusFilter}
            onChange={(e) => setStatusFilter(e.target.value)}
            className="min-h-[40px] rounded-md border border-lift-border bg-page-bg px-3 py-2 text-sm text-lift-text"
          >
            <option value="all">{t("briefing.filter_all_statuses")}</option>
        ...
        <div className="ml-auto flex items-end gap-4 text-xs text-muted">
          <span><strong className="text-emerald-400">{counts.ready}</strong> {t("briefing.readiness_ready").toLowerCase()}</span>
          <span><strong className="text-amber-400">{counts.stale}</strong> {t("briefing.readiness_stale").toLowerCase()}</span>
          <span><strong className="text-rose-400">{counts.missing}</strong> {t("briefing.readiness_missing").toLowerCase()}</span>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="rounded-lg border border-lift-border bg-surface p-6">
          <p className="text-sm text-muted">{t("briefing.empty")}</p>
        </div>
      ) : (
        <div className="overflow-x-auto rounded-lg border border-lift-border">
          <table className="w-full text-left text-sm">
            <thead className="bg-surface/60 text-xs text-muted">
              <tr>
                <th className="px-3 py-2" aria-label={t("briefing.select_aria")}>
                  {cycleId !== "all" && !activeSession && (
                    <input
                      type="checkbox"
                      aria-label={t("briefing.select_all_aria")}
                ...
                <th className="px-4 py-2 font-medium">{t("briefing.col_candidate")}</th>
                <th className="px-4 py-2 font-medium">{t("briefing.col_grade")}</th>
                <th className="px-4 py-2 font-medium">{t("briefing.col_status")}</th>
                <th className="px-4 py-2 font-medium">{t("briefing.col_tri")}</th>
                <th className="px-4 py-2 font-medium">{t("briefing.col_language_readiness")}</th>
                <th className="px-4 py-2 font-medium">{t("briefing.col_flags")}</th>
                <th className="px-4 py-2 font-medium">{t("dl.generated_at_prefix")}</th>
                  ...
                  <td className="px-4 py-2.5">
                    <FlagPill
                      ...
                      onClick={() => setDrawerCandidate({
                        id: r.id,
                        name: [r.first_name, r.last_name].filter(Boolean).join(" ") || t("committee.candidate_fallback"),
                      })}
                    />
                  </td>
