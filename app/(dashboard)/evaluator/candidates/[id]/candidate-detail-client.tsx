              ...
              return (
                <div key={dim} className="flex items-center gap-3">
                  <span className="w-32 text-sm capitalize flex items-center gap-1">
                    {dim.replace("_", " ")}
                    {TOOLTIPS[DIM_TOOLTIP_MAP[dim]] && <Tooltip content={TOOLTIPS[DIM_TOOLTIP_MAP[dim]]} />}
                  </span>
      ...
      {profile && (
        <div className="rounded-lg border border-primary/20 bg-primary/5 p-4">
          ...
          <div className="flex flex-wrap gap-4 text-xs">
            <span>TRI: <strong>{displayTriLabel(profile.tri_label as string)}</strong> ({Number(profile.tri_score ?? 0).toFixed(0)})</span>
...
function SupportPlanTab({ candidateId, candidateName }: { candidateId: string; candidateName: string }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-lg border border-lift-border bg-surface p-4">
        <h3 className="mb-3 text-sm font-semibold text-lift-text flex items-center gap-1.5">Week 1-2 Actions <Tooltip content={{ id: "sp_week12", title: "Week 1-2", body: "Immediate actions for the student's first two weeks. Check items off as your team completes them." }} /></h3>
        <div className="space-y-2">
      ...
      <div className="rounded-lg border border-lift-border bg-surface p-4">
        <h3 className="mb-3 text-sm font-semibold text-lift-text flex items-center gap-1.5">Month 1 Priorities <Tooltip content={{ id: "sp_month1", title: "Month 1", body: "Key goals for the first month. These build on the Week 1-2 actions and help establish the student's support rhythm." }} /></h3>
        <div className="space-y-2">
      ...
      <div className="rounded-lg border border-lift-border bg-surface p-4">
        <h3 className="mb-3 text-sm font-semibold text-lift-text flex items-center gap-1.5">Month 2-3 Checkpoints <Tooltip content={{ id: "sp_month23", title: "Month 2-3", body: "Milestones to check at the 60-90 day mark. These assess whether the student's transition support is working or needs adjustment." }} /></h3>
        <div className="space-y-2">
      ...
      {(plan.recommended_resources ?? []).length > 0 && (
        <div className="rounded-lg border border-lift-border bg-surface p-4">
          <h3 className="mb-3 text-sm font-semibold text-lift-text flex items-center gap-1.5">Recommended Resources <Tooltip content={{ id: "sp_resources", title: "Resources", body: "Mapped to your school's configured support resources. Priority indicates urgency: high = start immediately, medium = within first month, low = monitor and consider." }} /></h3>
