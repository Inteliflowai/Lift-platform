...
- **`CREATE OR REPLACE VIEW` resets `reloptions`**: Postgres does NOT preserve view-level options like `security_invoker = true` across a re-create. Migration 041 sets `security_invoker = true` on `trial_health` to clear the Supabase linter's `security_definer_view` ERROR; any later run of `CATCH_UP_PT_022_040.sql` (which contains `CREATE OR REPLACE VIEW trial_health` at line 211) silently strips that reloption and re-introduces the lint. The catch-up bundle now re-issues `ALTER VIEW trial_health SET (security_invoker = true);` inside the same transaction — keep it there. Verify via `SELECT relname, reloptions FROM pg_class WHERE relname = 'trial_health';` (linter checks `'security_invoker=true' = ANY(reloptions)`). Same trap applies to any future security-relevant view: every `CREATE OR REPLACE VIEW` site for that view must re-apply the reloption.
