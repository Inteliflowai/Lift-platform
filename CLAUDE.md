...
- **Demo candidate cleanup is soft-archive, not delete**: The first non-demo invite for a tenant flips `hidden_from_default_view = true` on every `is_demo` row (in `lib/invitations/trigger.ts`). Demos never get deleted — `/school/candidates` has a "Show sample candidates (N)" toggle that surfaces them again. Idempotent, so resends and bulk imports after the first real invite are no-ops here. Migration 044 is required for both columns.
