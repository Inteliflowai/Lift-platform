...
export function TRIGauge({ score, label, confidence, summary }: Props) {
  ...
  useEffect(() => {
    ...
    function animate(ts: number) {
      ...
      setAnimatedScore(Math.round(val * eased * 10) / 10);
  ...
  if (score === null) {
    return (
      <div className="rounded-xl border border-lift-border bg-surface p-6 text-center">
        <p className="text-sm text-muted">TRI score processing...</p>
      </div>
    );
  }

  return (
    <div className="rounded-xl border border-lift-border bg-surface p-6">
      <div className="flex flex-col items-center">
        ...
        <div className="mt-1 flex items-center gap-2">
          <span
            className={`rounded-full px-4 py-1 text-sm font-semibold capitalize ${
              LABEL_BG[label ?? ""] ?? "bg-gray-100 text-gray-600"
            }`}
          >
            {label ?? "—"}
          </span>
          <div className="relative">
            ...
            {showTooltip && (
              <div className="absolute bottom-6 left-1/2 z-10 w-64 -translate-x-1/2 rounded-lg border border-lift-border bg-white p-3 text-xs text-muted shadow-lg">
                The Transition Readiness Index is a composite indicator based on
                6 readiness dimensions. It is not a pass/fail score.
              </div>
            )}
          </div>
        </div>
        ...
        {/* Confidence */}
        {confidence && (
          <p className="mt-2 text-xs text-muted/70 capitalize">
...
export function TRIPill({
  ...
  return (
    <span className="inline-flex items-center gap-1.5">
      ...
      <span className="text-xs font-semibold" style={{ color: LABEL_COLORS[label ?? ""] }}>
